...
export default function AttendanceReportPage() {
    ...
    try {
      const res = await fetch(`/api/attendance/direct`, {
        ...
        body: JSON.stringify({
          ...
          remark: editingStudent.remark.trim() || 'แก้ไขโดยอาจารย์'
        })
      });
  ...
  return (
    <div className="min-h-screen flex flex-col bg-[#f0f7f4] font-sans text-slate-800">
      ...
      {/* 2. Navigation Tabs Bar (3 เมนูหลัก) */}
      ...
      <main className="flex-1 max-w-5xl w-full mx-auto p-4 md:p-8">
        ...
        {/* แถบสลับมุมมอง รายงานประจำวัน / สถิติรวม 18 สัปดาห์ */}
          ...
          <div className="animate-in fade-in duration-300">
            <div className="bg-white rounded-2xl p-6 shadow-sm border border-slate-200/80 mb-6 print:hidden">
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4 items-end">
                ...
                <div>
                  <Link
                    href={`/teacher/course/${courseId}/history?date=${selectedDate}`}
                        ...
                        return (
                          <tr key={item.id} className="hover:bg-slate-50/60 transition-colors">
                            ...
                            <td className="p-4">
                              ...
                              {/* แสดงหมายเหตุและ Badge ระบุสถานะการแก้ไข */}
                              {item.remark && (
                            ...
                            <td className="p-4 text-center">
                              <select
                                ...
                                className={`text-xs font-bold px-3 py-1.5 rounded-xl border outline-none cursor-pointer transition-all ${item.status === 'มาเรียน' ? 'bg-emerald-50 text-emerald-700 border-emerald-200' :
                    ...
                    {loading ? (
                      ...
                      weeksList.map((week) => (
                        <tr
                          ...
                          className={`transition-colors cursor-pointer ${week.isChecked ? 'hover:bg-emerald-50/40 bg-white' : 'hover:bg-slate-50/80 bg-slate-50/20'
                            }`}
                          title={week.isChecked ? `คลิกเพื่อดูรายชื่อนักศึกษาในสัปดาห์ที่ ${week.weekNumber}` : 'ยังไม่มีการเช็คชื่อในสัปดาห์นี้'}
                        >
                          <td className="p-4 text-center">
                            <span className={`inline-flex items-center justify-center w-8 h-8 rounded-xl font-bold text-xs ${week.isChecked
            ...
            <form onSubmit={handleSaveStatusModal} className="space-y-4">
              ...
              <div>
                ...
                <div className="flex flex-wrap gap-1.5">
                  {['เช็คชื่อรอบที่ 2', 'แก้ไขโดยอาจารย์', 'ลากิจ', 'ลาป่วย', 'เช็ครอบเก็บตก'].map((tag) => (
                    <button
                      key={tag}
                      type="button"
